...
router.get('/mensaje', auth.comun, async (req, res, next) => {
    try {
        const { idChat } = req.query
        if (idChat) {
            await pool.query(`SELECT idmensajes,idUsuarios,descripcion, CONVERT_TZ(fecha, '+00:00', '-03:00') as fecha FROM mensajes WHERE idChat = ? ORDER BY fecha ASC;`, idChat, (err, result) => {
                handleDBerrors(err, res, () => {
                    if (result.length > 0) {
                        console.log(result)
                        res.status(200).json({
