...
router.get('/user', auth.comun, async (req, res, next) => {
    ...
    const iduser = _idUsuarios == req.query.iduser ? null : req.query.iduser;
    try {
        if (!iduser) {
        ...
        if (Object.keys(userModify).length > 1 && userModify.constructor === Object) {
            try {
                if (userModify.password) {
                    userModify.password = await helpers.encryptPassword(userModify.password);
                }
...
router.get('/publicacion', auth.comun, async (req, res, next) => {
    ...
    try {
        let querySQL = 'SELECT idUsuarios FROM PUBLICACIONES WHERE idPublicaciones = ? ';
        const tipo = await auth.getUserType(token);
        if (tipo === 1) querySQL += 'and disponible=1';
        const result = await pool.query(querySQL, idPub);
        if (result.length > 0) {
...
router.put('/ban', auth.adm, async (req, res, next) => {
    const { iddenuncias, flag } = req.body;
    const aux = (flag == 'true')
    console.log(iddenuncias, flag, aux)
    try {
        const estadoDenuncia = await pool.query('SELECT estado FROM denuncias WHERE iddenuncias=?', iddenuncias)
        if (estadoDenuncia[0]) {
            if (aux) {
                await pool.query('call banear(?)', iddenuncias, (err, result) => {
                    handleDBerrors(err, res, () => {
                        if (result[0]) {
                            res.status(200).json({
                                proceso: 'BAN',
                                message: result[0][0].Mensajito
                            });
                        } else {
                            res.status(400).json({
                                message: 'La operación no se pudo concretar'
                            })
                        }
                    })
                });
            ...
            res.status('404').json({
                message: 'El recurso ya fue revisado'
            })
        }
    } catch (error) {
        next(error)
