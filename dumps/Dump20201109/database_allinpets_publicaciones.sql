...
CREATE TABLE `publicaciones` (
  ...
  `lastmfecha` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `disponible` tinyint(1) NOT NULL DEFAULT '1',
  PRIMARY KEY (`idPublicaciones`),
  KEY `fk_Publicaciones_Usuarios_idx` (`idUsuarios`),
  CONSTRAINT `fk_Publicaciones_Usuarios` FOREIGN KEY (`idUsuarios`) REFERENCES `usuarios` (`idUsuarios`) ON DELETE CASCADE
